...
def _message_event(adapter, text="hello", message_id="e1", chat_id=CHANNEL, chat_type="group",
    ...
    source = adapter.build_source(
        chat_id=chat_id, chat_name=chat_id, chat_type=chat_type, user_id=user_id,
        user_name="other",
    )
    return MessageEvent(
...
async def _drain_sessions(adapter, rounds=8):
    ...
    """
    for _ in range(rounds):
        session = [t for t in list(getattr(adapter, "_session_tasks", {}).values())
                   if not t.done()]
...
class TestReactionPrimitives:
    ...
    async def test_add_uses_event_and_emoji_no_channel(self):
        ...
        assert cli.calls == [(["reactions", "add", "--event", "e1", "--emoji", EYES], None)]
    ...
    async def test_remove_uses_event_and_emoji_no_channel(self):
        ...
        assert cli.calls == [(["reactions", "remove", "--event", "e1", "--emoji", EYES], None)]
...
class TestTransitionCoordinator:
    ...
    async def test_success_sequence_ordered(self):
        ...
        assert cli.reaction_ops() == [
            ("add", EYES), ("remove", EYES), ("add", BRAIN),
            ("remove", BRAIN), ("add", OK),
        ]
    ...
    async def test_cancelled_removes_without_terminal(self):
        ...
        await _drain_reactions(adapter)
        assert cli.reaction_ops() == [
            ("add", EYES), ("remove", EYES), ("add", BRAIN), ("remove", BRAIN),
        ]
        assert key not in adapter._reaction_lifecycle
    ...
    async def test_transitions_serialize_never_stack(self):
        ...
        start_task = asyncio.create_task(
            adapter.on_processing_start(_evt_with(key))
        )
    ...
    async def test_hooks_noop_for_unknown_event(self):
        ...
        await adapter.on_processing_complete(_evt_with((CHANNEL, "unknown")), _outcome_success())
        await _drain_reactions(adapter)
        assert cli.reaction_ops() == []
    ...
    async def test_unauthorized_sender_gets_no_lifecycle(self):
        ...
        await adapter._dispatch_message(
            text="hello", chat_id=CHANNEL, chat_type="group", user_id=OTHER_PUBKEY,
            user_name="other", message_id="e1", created_at=1000,
        )
        await _drain_reactions(adapter)
        assert cli.reaction_ops() == []
...
class TestDispatchWiring:
    ...
    async def test_command_message_gets_no_reaction(self):
        ...
        await adapter._dispatch_message(
            text="/status", chat_id=CHANNEL, chat_type="group", user_id=OTHER_PUBKEY,
            user_name="other", message_id="e1", created_at=1000,
        )
        await _drain_reactions(adapter)
        assert cli.reaction_ops() == []
...
class TestBaseHookIntegration:
    async def _run_turn(self, adapter, handler, text="hello"):
        adapter._message_handler = handler
        await adapter._dispatch_message(
            text=text, chat_id=CHANNEL, chat_type="group", user_id=OTHER_PUBKEY,
            user_name="other", message_id="e1", created_at=1000,
        )
        for _ in range(8):
            session = [t for t in list(getattr(adapter, "_session_tasks", {}).values())
